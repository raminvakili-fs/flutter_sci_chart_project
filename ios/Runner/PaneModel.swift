...
class RsiPaneModel: BasePaneModel {
    var rsiDataSeries : SCIXyDataSeries?
    
    init(prices: SCDPriceSeries) {
        super.init(title: RSI, yAxisTextFormatting: "0.0", isFirstPane: false)

        rsiDataSeries = SCIXyDataSeries(xType: .date, yType: .double)
        rsiDataSeries?.seriesName = "RSI"
        rsiDataSeries?.append(x: prices.dateData, y: SCDMovingAverage.rsi(prices, period: 10))
    ...
    func updateData(prices: SCDPriceSeries) {
        rsiDataSeries?.clear()
        rsiDataSeries?.append(x: prices.dateData, y: SCDMovingAverage.rsi(prices, period: 10))
    }
}
...
class MacdPaneModel: BasePaneModel {
    ...
    var histogramDataSeries : SCIXyDataSeries?
    var macdDataSeries : SCIXyyDataSeries?
    ...
    init(prices: SCDPriceSeries) {
        ...
        histogramDataSeries = SCIXyDataSeries(xType: .date, yType: .double)
        ...
        macdDataSeries = SCIXyyDataSeries(xType: .date, yType: .double)
